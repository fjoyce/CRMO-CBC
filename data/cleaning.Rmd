...
Start year = 95 (1994)
End year = 120 (2019) [year prior to December count date]


CSV, sort Taxonomic

[Can do this with code after importing]
...
```{r}
CRMO_cbc <- read_csv(here("data/CRMO-1994-2024_pre-cleaned.csv"))
```
...
```{r}
#this just pulls out the values in the species column for the first row
CRMO_cbc %>% 
  filter(row_number() == 1) %>% 
  pull(species)
```
...
```{r}
write_csv(CRMO_cbc, "CRMO-CBC-2024-cleaned.csv")
```
